...
//function to create the html tags and inner html for the repositories retrieved from the query
  ...
  createRepositories = (repository) => {
    ...
    if(repository.isPrivate){
      ...
      privateSpan.classList.add('private-span');
      privateSpan.innerHTML = "private";
      repoDetailsDiv.append(privateSpan);
    }
...
//the map function to display the queries on the screen
  repository.map(createRepositories);
}

getRepos();
function display(){
    var hamburgerToggle = document.getElementById("nav-links");
...
let profilePicDiv = document.getElementById("nav-profile-pic");
profilePicDiv.addEventListener("click", function () {
 ...
 let expandSearchBox = document.getElementById('expandSearchBox');
  ...
  expandSearchBox.classList.add('active');
  let keySlash = document.getElementsByClassName('search-key-slash');
  keySlash[0].classList.add('active');
 })
